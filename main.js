...
class CircuitBackground {
  ...
  // Perbarui palet warna (dipanggil saat tema berganti) tanpa mengacak ulang jalur
  setPalette(colors) {
 ...
 */
class FloatingClock {
  constructor(rootId, timeId, secId, dateId, ringId) {
    this.root = document.getElementById(rootId);
    this.timeEl = document.getElementById(timeId);
    this.secEl = document.getElementById(secId);
    this.dateEl = document.getElementById(dateId);
    this.ringEl = document.getElementById(ringId);
    if (!this.root) return;

    this.circumference = 2 * Math.PI * 19.5; // ~122.5
  ...
  tick() {
    ...
    if (this.timeEl) {
      this.timeEl.firstChild.nodeValue = `${h}:${m}`;
    }
    if (this.secEl) {
      this.secEl.textContent = `:${this.pad(s)}`;
    }
    if (this.dateEl) {
      this.dateEl.textContent = `${this.dayNames[now.getDay()]}, ${now.getDate()} ${this.monthNames[now.getMonth()]}`;
    }
    if (this.ringEl) {
      const progress = s / 60;
  ...
  bindScrollCompact() {
    ...
    window.addEventListener('scroll', () => {
      ...
      if (delta > 4) {
        this.root.classList.add('fc-compact');
      }

      clearTimeout(scrollTimeout);
      scrollTimeout = setTimeout(() => {
        this.root.classList.remove('fc-compact');
      }, 650);
    }, { passive: true });
  }
}
...
/**
 * Kelola pergantian tema (gelap/terang) dan simpan pilihan pengguna
 */
class ThemeManager {
  constructor(toggleId, circuitBg) {
    this.root = document.documentElement;
    this.toggle = document.getElementById(toggleId);
    this.circuitBg = circuitBg || null;
    this.storageKey = 'nic-theme';

    this.palettes = {
      dark: ['#2ee6ff', '#ff3ec8', '#ffc93c'],
      light: ['#6fd6d1', '#e79bc7', '#f0b96b']
    };

    this.applyPalette(this.getCurrentTheme());
  ...
  applyPalette(theme) {
    if (this.circuitBg) {
      this.circuitBg.setPalette(this.palettes[theme]);
    }
  }
...
class App {
  static init() {
    ...
    const circuitBg = new CircuitBackground('circuit-canvas');
    new ThemeManager('themeToggle', circuitBg);
    ...
    new MagneticHover();
    new TiltCardManager();
    new FloatingClock('floatClock', 'fcTime', 'fcSec', 'fcDate', 'fcRingProg');
  }
}

/* ============ AUTO-HIDE SCROLLBAR ============ */
class AutoHideScrollbar {
  constructor(edgeThreshold = 24, hideDelay = 900) {
    this.root = document.documentElement;
    this.edgeThreshold = edgeThreshold;
    this.hideDelay = hideDelay;
    this.hideTimer = null;
    this.pointerNearEdge = false;
    ...
    window.addEventListener('touchmove', () => this.show(), { passive: true });
  }

  handlePointer(clientX) {
    const nearEdge = clientX >= window.innerWidth - this.edgeThreshold;
    if (nearEdge !== this.pointerNearEdge) {
      this.pointerNearEdge = nearEdge;
      if (nearEdge) {
        this.show(true);
      } else {
        this.scheduleHide();
  ...
  show(keepIfEdge) {
    this.root.classList.add('scrollbar-visible');
    clearTimeout(this.hideTimer);
    if (!this.pointerNearEdge || !keepIfEdge) {
      this.scheduleHide();
    }
  }
  ...
  scheduleHide() {
    ...
    this.hideTimer = setTimeout(() => {
      if (!this.pointerNearEdge) {
        this.root.classList.remove('scrollbar-visible');
      }
    }, this.hideDelay);
  }
}
